import { renderTechIcon } from "../utils/iconMaps"

function InlineLogoItem({ item }) {
  return (
    <li className="inline-flex items-center gap-1.5">
      {renderTechIcon(item, "h-4 w-4 text-zinc-500 dark:text-zinc-400")}
      <span>{item}</span>
    </li>
  )
}
    ...
    <section id="tech-stack" className="scroll-mt-20 py-8 md:py-9">
      ...
      <div className="mt-5 space-y-6">
        <article className="rounded-3xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <div className="mt-4 space-y-6 text-sm md:text-base text-zinc-700 dark:text-zinc-300">
            <div>
              <p className="font-medium text-zinc-900 dark:text-zinc-100">Languages</p>
              <ul className="mt-1 flex flex-wrap gap-x-4 gap-y-1">
                {content.languages.map((item) => (
                  <InlineLogoItem key={item} item={item} />
                ))}
              </ul>
            </div>
            <div>
